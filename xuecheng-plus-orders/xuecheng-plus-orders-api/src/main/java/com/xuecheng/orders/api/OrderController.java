...
public class OrderController {
    ...
    public PayRecordDto generatePayCode(@RequestBody AddOrderDto addOrderDto) {
        ...
        lqw.eq(StringUtils.isNotEmpty(addOrderDto.getOrderName()), XcPayRecord::getOrderName, addOrderDto.getOrderName());
        lqw.eq(StringUtils.isNotEmpty(user.getId()), XcPayRecord::getUserId, user.getId());
        XcPayRecord xcPayRecord = xcPayRecordMapper.selectOne(lqw);
        PayRecordDto payRecordDto;
        if (xcPayRecord != null && xcPayRecord.getStatus().equals("601002")){
            payRecordDto = new PayRecordDto();
            BeanUtils.copyProperties(xcPayRecord, payRecordDto);
        }else {
            orderService.createOrder(user.getId(), addOrderDto);
            payRecordDto = orderService.createOrder(user.getId(), addOrderDto);
        }
        ...
        try {
            ...
            String url = String.format(serverUrl, payRecordDto.getPayNo());
...
//        alipayRequest.setReturnUrl("http://domain.com/CallBack/return_url.jsp");
        alipayRequest.setNotifyUrl(randomUrl + "/orders/paynotify");//在公共参数中设置回跳和通知地址
