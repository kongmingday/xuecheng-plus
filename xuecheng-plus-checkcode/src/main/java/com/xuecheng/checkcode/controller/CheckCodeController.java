...
public class CheckCodeController {
    ...
    public void findPasswordCheckCode(@RequestParam("param1") String param){
        if (param.contains("@")){
            String generate = numberCodeGenerator.generate(4);
            emailService.sendSimpleMail(sendEmail, param,
                    "xuecheng-plus寻回密码",
                    "您的验证码为: " + generate);
            redisCheckCodeStore.set(findKey + ":" + param, generate, 180);
        }else {
            return;
        }
    }
